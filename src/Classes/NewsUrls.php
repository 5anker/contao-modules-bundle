...
class NewsUrls
{
	...
	public function getPageIdFromUrl($arrFragments)
	{
		...
		} elseif (count($arrFragments) == 1) { // otherwise check if there is exactly only one fragment
		...
		if ($alias) {
			...
			if (($objNews = \NewsModel::findByAlias($alias)) !== null) {
				...
				if (($objTarget = \PageModel::findWithDetails($objNews->getRelated('pid')->jumpTo)) !== null) {
					...
					return [ $objTarget->alias, 'auto_item', $objNews->alias ];
				}
			}
		}
	...
	public function parseArticles($objTemplate, $arrArticle, $objModule)
	{
		...
		if (stripos($strRequest, $arrArticle['alias']) !== 0) {
			...
			switch ($redirectType) {
				...
				case 'canonical':
					$GLOBALS['TL_HEAD'][] = '<link rel="canonical" href="'. $strUrl .'">';
					break;
				...
				case 302:
				case 303:
					\Controller::redirect($strUrl . $strQuery, $redirectType);
					break;
				default:
					\Controller::redirect($strUrl . $strQuery, 301);
			}
		}
	}
}
